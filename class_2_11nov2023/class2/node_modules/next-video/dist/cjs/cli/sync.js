...
var import_logger = __toESM(require("../logger.js"), 1);
...
function watcher(dir) {
  ...
  watcher2.on("add", async (filePath, stats) => {
    const relativePath = import_node_path.default.relative((0, import_node_process.cwd)(), filePath);
    const newAsset = await (0, import_assets.createAsset)(relativePath, {
      size: stats?.size
    });
    if (newAsset) {
      import_logger.default.add(`New file found: ${filePath}`);
      return (0, import_process.callHandler)("local.video.added", newAsset, getCallHandlerConfig());
    }
  });
}
function getCallHandlerConfig() {
  return JSON.parse(import_node_process.env["__NEXT_VIDEO_OPTS"] ?? "{}");
}
async function handler(argv) {
  const directoryPath = import_node_path.default.join((0, import_node_process.cwd)(), argv.dir);
  try {
    const files = await (0, import_promises.readdir)(directoryPath);
    ...
    const newFileProcessor = async (file) => {
      ...
      const absolutePath = import_node_path.default.join(directoryPath, file);
      const relativePath = import_node_path.default.relative((0, import_node_process.cwd)(), absolutePath);
      const stats = await (0, import_promises.stat)(absolutePath);
      const newAsset = await (0, import_assets.createAsset)(relativePath, {
        size: stats.size
      });
      if (newAsset) {
    ...
    const existingFileProcessor = async (file) => {
      ...
      if (assetStatus && ["sourced", "pending", "uploading", "processing"].includes(assetStatus)) {
        return (0, import_process.callHandler)("local.video.added", existingAsset, getCallHandlerConfig());
      }
    };
    ...
    if (processed.length > 0) {
      ...
      import_logger.default.success(`Processed (or resumed processing) ${processed.length} video${s}`);
    }
  } catch (err) {
